...
if($key){
	...
	$data = curlWrap("/channels/voice/phone_numbers.json",NULL,"GET");
		...
		foreach ($data->phone_numbers as $phone_number) {
			...
			//Replace the value
			echo "Setting the voicemail for number ".$phone_number->number."<br/>";
			...
			if($greetings){
				...
				echo "Greeting is already set up, ignoring this update.<br/>";
			}
		}
}else{
	echo "It's not time to change the voicemail greeting.<br/>";
}
...
function curlWrap($url, $json, $action){
	...
	if($action =="PUT"){
		echo "Server response:<br/>" .$output. "<br/>";
	}
	return $decoded;
}
...
function generateGreetingsArray($current_greetings,$key){
	...
	if($key=='start'){
		echo "Setting up the start of the day voicemail<br/>";
		...
		if($res === false){
			echo "END_GREETING ".END_GREETING." was not found on this number.<br/> Current Greeting ID's:<br/>";
			...
			if($ver === false){
				echo "START_GREETING ".START_GREETING." was not found either on this number.<br/>";
				echo "Adding START_GREETING ".START_GREETING." into array.<br/>";
				$current_greetings[] = START_GREETING;
			}else{
				return false;
			}
		}else{
			echo "END_GREETING found at index " . $res . "<br/>Current Greeting ID's:<br/>";
			var_dump($current_greetings); 
			// I replace the value of END_GREETING with START_GREETING
			echo "Replacing greeting ".$current_greetings[$res]." from array with ".START_GREETING.".<br/>";
			$current_greetings[$res] = START_GREETING;
		}
	}else{
		echo "Setting up the end of the day voicemail<br/>";
		...
		if($res === false){
			echo "START_GREETING ".START_GREETING." was not found on this number.<br/> Current Greeting ID's:<br/>";
			var_dump($current_greetings);
			// Checking that the Array doesn't contain the END_GREETING already
			$ver = array_search(END_GREETING, $current_greetings);
			if($ver === false){
				echo "END_GREETING ".END_GREETING." was not found either on this number.<br/>";
				echo "Adding END_GREETING ".END_GREETING." into array.<br/>";
			...
			// I replace the value of END_GREETING with START_GREETING
			echo "START_GREETING found at index " . $res . "<br/>Current Greeting ID's:<br/>";
			var_dump($current_greetings); 
			echo "Replacing greeting ".$current_greetings[$res]." from array with ".END_GREETING.".<br/>";
			$current_greetings[$res] = END_GREETING;
		}
	}
	echo "New greetings array is: <br/>";
	var_dump($current_greetings);
